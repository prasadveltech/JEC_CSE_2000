...
class Member:
    ...
    def __init__(self, name, first, period):
        ...
        #print("name => {} , first => {} , period {}".format(name,first,period))
        self.name = name
        self.words = {}
...
class Chat:
    ...
    @staticmethod
    def strdate(s):
        #print('File string = strdate(s) {} Message => {}', dt.date(int(s[7:11]), int(s[4:6]), int(s[1:3])), int(s[13:15]), s)
        """Extracts date and hour out of given string
        
        old ==> return dt.date(2000+int(s[6:8]), int(s[3:5]), int(s[:2])), int(s[10:12])
        
        """
        return dt.date(int(s[7:11]), int(s[4:6]), int(s[1:3])), int(s[13:15])
    ...
    def _rmnl(self):
        ...
        for msg in self.chat:
            ...
            if len(msg) > 23 and msg[3] == msg[6] == '/' and msg[11:13] == ', ' and msg[15] == msg[18] == ':' and msg[21:23] == '] ':
                if prev: res.append(prev)
                prev = msg
            else:
                if prev: prev = prev[:-1] + ' ' + msg
        #print("File read lines ===>", res)
        res.append(prev)
        self.chat = res
    ...
    def process(self):
        ...
        for line in self.chat:
            if line.find("changed the subject to ") != -1 or line.find("  added ") != -1 or line.find("Roll No : ") != -1 :
                continue
            try:
                #print("hour ==> {}".format(line[13:15]))
                hour = int(line[13:15])
                #print("date ==> {}".format(Chat.shftfive(*Chat.strdate(line))))
                date = Chat.shftfive(*Chat.strdate(line))
                #print("new line var ==> {}".format(line[27:]))
                line = line[23:]
                #print("name ==> {}".format(line[:line.index(':')]))
                name = line[:line.index(':')]
                #print("another new line var ==> {}".format(line[line.index(': ') + 2:]))
                line = line[line.index(': ') + 2:]
            except ValueError:
                #print("Value Error {} , {} , {} , {}".format(name, date, period, line))
                pass  # ignore corrupted messages
            else:
                if all(member.name != name for member in members):
                    #print("New Member {} , {} , {} , {} , {}".format(name, date, first, period, line))
                    members.append(Member(name, date, period))
                for member in members:
                    if member.name == name:
                        #print("Member.add_message {} , {} , {} , {}".format(line, (date-first).days, date.weekday(), hour))
...
def message_trend(members):
    ...
    #print("members[0].days ==> ", members[0].days)
    #print("period ==> ", len(members[0].days))
    ...
    """Legend issue needs to be fixed"""
...
def weekday_avg(members):
    ...
    #print("weekday_avg {} , {} , {} , {} , {}".format(frst_wd, last_wd, wd_sum_msgs, period, wd_count))
    wd_avg_msgs = tuple(map(lambda e, a: e / a if a != 0 else 0, wd_sum_msgs, wd_count))
    #print("weekday_avg - wd_avg_msgs {}".format(wd_avg_msgs))
    ...
    plt.title('Average Messages per Day', y=1.03, weight='bold')
    plt.ylabel('# Messages')
...
if __name__ == '__main__':
    ...
    # prepare window
    fig = plt.figure()
    fig.canvas.set_window_title('JEC CSE 2000 Whatsapp Group Analyzer')
    # show plots
